...
int brightnes=2;
int briLevel[5]={40,54,70,100,140};
...
void setup() {
    ...
    M5Cardputer.Display.setBrightness(briLevel[brightnes]);
...
for(int i=0;i<8;i++){
  ...
  M5Cardputer.Display.pushImage(0,0,240,135,(uint16_t*)sprite.getPointer()); 
}
...
void loop() {
  ...
  acc2=map((int)speed,gearMax[gear-1]-10,gearMax[gear],0,4);
   if(digitalRead(0)==0)
   {if(rpm<260)rpm=rpm+accR[acc];;
   if(speed<gearMax[gear]) speed=speed+accS[acc2];
   }else {
    if(rpm>0) rpm=rpm-4;
    if(rpm<0) rpm=0;
    ...
    if (M5Cardputer.Keyboard.isChange()) {
      ...
      {M5Cardputer.Speaker.tone(5000, 300);}
      ...
      {rightD=!rightD;
      if(rightD==0) rightBlink=18;
      if(rightD){leftD=0; leftBlink=20;}
      }

         if (M5Cardputer.Keyboard.isKeyPressed(','))
      {leftD=!leftD;
      if(leftD==0) leftBlink=18;
      if(leftD){rightD=0; rightBlink=20;}
      }
      ...
      {lightsOn=!lightsOn;}
       ...
       {brightnes++; if(brightnes==5) brightnes=0;
        M5Cardputer.Display.setBrightness(briLevel[brightnes]);}

      if (M5Cardputer.Keyboard.isKeyPressed('z'))
      {speed++;
      if(speed>149) speed=0;}
      }
